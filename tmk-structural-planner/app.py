...
def write_query(product: int, stage: str) -> None:
    try:
        st.query_params.update({"product": str(product), "stage": str(stage)})
    except Exception:
        pass
        ...
        if stage_key == "E":
            ...
            for row_i, row in enumerate(layout):
                for col_i, p in enumerate(row):
                    if p in stage_products:
                        x = startx + col_i * stepx
                        yy = y + (row_i - 1) * stepy
                        positions[p] = (x, yy)
                        placed.add(p)

            remaining = [p for p in stage_products if p not in placed]
            if remaining:
                left = 200
                right = width - 80
                step = 0 if len(remaining) == 1 else (right - left) / (len(remaining) - 1)
                xs = [width / 2] if len(remaining) == 1 else [left + i * step for i in range(len(remaining))]
                for p, x in zip(remaining, xs):
                    positions[p] = (x, y)
        else:
...
def build_world_map_svg(products: List[int], selected_product: int, selected_stage: str) -> str:
    ...
    svg = [
        ...
        .hub-text { font: 800 20px Arial, sans-serif; fill: white; pointer-events: none; }
    ...
    svg.append(
        f'''
        <g>
            <rect x="{legend_x}" y="{legend_y}" width="290" height="76" rx="16" fill="rgba(255,255,255,0.92)" stroke="#e2e8f0" stroke-width="1.2"/>
...
def build_radial_svg(product: int, product_routes: List[Route], product_exits: List[Route], color: str) -> str:
    ...
    svg = [
        ...
        .hub-text { font: 800 42px Arial, sans-serif; fill: white; }
